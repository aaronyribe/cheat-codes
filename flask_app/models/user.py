...
class User:
    ...
    @classmethod
    def get_all(cls):
        query = "SELECT * FROM cheat_code_schema.users;"
    ...
    def get_by_email(cls,data):
        query = "SELECT * FROM cheat_code_schema.users WHERE email = %(email)s;"
    ...
    def get_by_id(cls,data):
        query = "SELECT * FROM cheat_code_schema.users WHERE id = %(id)s;"
        results = connectToMySQL(cls.db).query_db(query,data)
        return cls(results[0])
    ...
    def validate_register(user):
        ...
        if type(results) != bool and len(results) >= 1:
